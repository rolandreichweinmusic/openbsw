...
void irq_add_shared_handler(uint num, irq_handler_t handler, uint8_t order_priority) {
    ...
    if (!is_shared_irq_raw_handler(vtable_handler)) {
        ...
        *slot = slot_data;
        ...
        assert(!((((uintptr_t)remove_thumb_bit((void*)vtable_handler)) - ((uintptr_t)irq_handler_chain_slots)) % sizeof(struct irq_handler_chain_slot)));
        struct irq_handler_chain_slot *prev_slot = NULL;
        struct irq_handler_chain_slot *existing_vtable_slot = (irq_handler_chain_slot*)remove_thumb_bit((void*)vtable_handler);
