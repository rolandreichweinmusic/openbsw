...
uint32_t getSystemTicks32Bit(void) { return static_cast<uint32_t>(to_us_since_boot(get_absolute_time())); }
...
uint32_t getSystemTimeUs32Bit(void) { return static_cast<uint32_t>(to_us_since_boot(get_absolute_time())); }
...
{
    return static_cast<uint32_t>(getSystemTimeMs());
}
